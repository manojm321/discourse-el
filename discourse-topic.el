...
(defun discourse-topic-populate-topic (topic)
  ...
  (let* ((topic-buf (get-buffer-create "*discourse topic info*")))
    (with-current-buffer topic-buf
      (let* ((posts (cdr-safe (assoc 'posts (assoc 'post_stream response))))
             (fmt-posts (mapcar #'discourse-topic--format-post posts))
             (id (cdr-safe (assoc 'id response)))
             (title (cdr-safe (assoc 'title response)))
             (posts-count (cdr-safe (assoc 'posts_count response)))
             (views (cdr-safe (assoc 'views response)))
             (tags (cdr-safe (assoc 'tags response)))
             (inhibit-read-only t))
        (erase-buffer)
        (insert "<!DOCTYPE html>\n<html>\n<body>\n")
